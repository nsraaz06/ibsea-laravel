...
<div class="bg-navy-accent dark:bg-slate-950 min-h-screen pt-[72px] relative overflow-x-hidden">
    ...
    <div class="fixed top-[88px] right-6 z-[80] flex flex-col gap-3">
        ...
        <a href="{{ asset($resource->file_path) }}" download class="w-12 h-12 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-2xl flex items-center justify-center hover:bg-white hover:text-navy-accent transition-all shadow-2xl group">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const url = '{{ asset($resource->file_path) }}';
        ...
        loadingTask.promise.then(pdf => {
            ...
            // Optimization: If it's a huge PDF (>50 pages), we load structure in chunks
            for (let i = 1; i <= pdf.numPages; i++) {
                const pageWrapper = document.createElement('div');
                pageWrapper.className = 'pdf-page-wrapper bg-white shadow-2xl rounded-sm overflow-hidden relative min-h-[400px] md:min-h-[800px] transition-transform duration-500';
...
.catch(err => {
            ...
            container.innerHTML = `
                <div class="bg-red-500/10 border border-red-500/20 p-12 rounded-3xl text-center">
                    ...
                    <a href="{{ route('public.resources.index') }}" class="inline-block bg-navy-accent text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest">Return to Archive</a>
                </div>
            `;
...
<style>
    ...
    /* Ensure canvases don't create layout shifts */
    canvas {
