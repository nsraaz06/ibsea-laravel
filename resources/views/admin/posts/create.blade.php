...
<div class="p-10">
    <header class="flex justify-between items-center mb-10">
        <div>
            <h2 class="text-3xl font-bold text-primary tracking-tight">Compose Mission Dispatch</h2>
            <p class="text-slate-500 font-semibold italic">Initialize new institutional intelligence broadcast.</p>
        </div>
        ...
        <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="bg-slate-50/80 backdrop-blur-sm p-8 rounded-[2rem] border border-slate-100 space-y-8 shadow-inner">
                    <div class="space-y-4">
                        ...
                        <select name="status" class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-accent/10 focus:border-accent transition-all shadow-sm cursor-pointer appearance-none">
                            <option value="Published">Broadcast (Published)</option>
                            <option value="Draft" selected>Archive (Draft)</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <select name="category" class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-accent/10 focus:border-accent transition-all shadow-sm cursor-pointer appearance-none">
                            <option value="News">News Dispatch</option>
                            <option value="Blog" selected>Leadership Blog</option>
                    ...
                    <label class="flex items-center justify-between cursor-pointer group bg-white/80 p-6 rounded-2xl border border-transparent hover:border-accent/20 transition-all shadow-sm">
                        ...
                        <div class="relative inline-flex items-center">
                            <input type="checkbox" name="show_on_slider" value="1" class="sr-only peer">
                ...
                <div class="bg-primary/5 p-8 rounded-[2rem] border border-primary/10 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-wider block px-1">Meta Title</label>
                        <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-wider block px-1">Meta Keywords</label>
                        <input type="text" name="meta_keywords" value="{{ old('meta_keywords') }}" class="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-wider block px-1">Meta Description</label>
                        <textarea name="meta_description" rows="3" class="w-full bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all">{{ old('meta_description') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-10">
                <div class="space-y-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Enter compelling dispatch headline..." class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-8 py-5 text-lg font-bold text-slate-800 placeholder:text-slate-300 focus:bg-white focus:ring-4 focus:ring-accent/10 focus:border-accent transition-all shadow-inner">
                </div>
                ...
                <div class="pt-10 flex justify-end">
                    <button type="submit" class="bg-accent text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-premium hover:bg-primary hover:-translate-y-1 active:scale-95 transition-all flex items-center gap-4">
                        Initialize Dispatch <span class="material-icons text-sm">rocket_launch</span>
                    </button>
                </div>
            </div>
        </form>
