...
<div class="p-10">
    <header class="flex justify-between items-center mb-10">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 uppercase tracking-tight">Edit Institutional Role</h2>
            <p class="text-slate-500 font-semibold">Update the designation and hierarchy for the {{ $role->role_name }} position.</p>
        </div>
        ...
        <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="role_name" value="{{ old('role_name', $role->role_name) }}" required class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all">
                </div>

                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Hierarchy Level</label>
                    <input type="number" name="hierarchy_level" value="{{ old('hierarchy_level', $role->hierarchy_level) }}" required class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all">
                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2 px-1">Lower values indicate higher strategic rank.</p>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-8 rounded-3xl space-y-4">
                ...
                <label class="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 cursor-pointer hover:border-primary transition-all">
                    ...
                    <div class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="show_in_leadership" value="1" {{ old('show_in_leadership', $role->show_in_leadership) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-2">
                        ...
                        <label class="relative flex items-center p-4 bg-white rounded-2xl border border-slate-100 cursor-pointer hover:border-primary group transition-all">
                            <input type="radio" name="card_display_pattern" value="{{ $value }}" {{ old('card_display_pattern', $role->card_display_pattern) == $value ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-full">
            ...
            <div class="pt-8 flex justify-end">
                <button type="submit" class="bg-primary text-white px-12 py-5 rounded-3xl font-bold text-xs uppercase tracking-widest shadow-xl shadow-primary/20 hover:bg-primary/90 active:scale-95 transition-all flex items-center gap-3">
                    Refine Designation <span class="material-icons text-sm">verified_user</span>
                </button>
            </div>
        </form>
    </div>
