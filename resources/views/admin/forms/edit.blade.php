...
<div class="p-10">
    <header class="flex justify-between items-center mb-10">
        <div>
            <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Refine Form Protocol</h2>
            <p class="text-slate-500 font-medium">Update fields and settings for protocol: <span class="text-primary font-bold">{{ $form->title }}</span></p>
        </div>
    ...
    <form action="{{ route('admin.forms.update', $form) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" rows="3" placeholder="Brief context for the user..." 
                            class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all">{{ old('description', $form->description) }}</textarea>
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Success Message</label>
                        <input type="text" name="settings[success_message]" value="{{ old('settings.success_message', $form->settings['success_message'] ?? 'Intelligence logged successfully.') }}" 
                            class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Featured Banner Image</label>
                        <p class="text-[10px] text-slate-400 px-1 -mt-1">Recommended: 1920×1080px. Leave blank to keep existing.</p>
                        @if(!empty($form->settings['featured_image']))
                            <img src="{{ asset('storage/' . $form->settings['featured_image']) }}" class="w-full rounded-xl object-cover max-h-24" alt="Current Banner">
                        @endif
                        <div class="relative border-2 border-dashed border-slate-200 rounded-2xl p-4 text-center cursor-pointer hover:border-primary transition-all" onclick="document.getElementById('featured_image_input').click()">
                            ...
                            <p class="text-xs font-bold text-slate-400">Click to replace image</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="settings[confirmation_template_id]" 
                            ...
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ (old('settings.confirmation_template_id', $form->settings['confirmation_template_id'] ?? '')) == $template->id ? 'selected' : '' }}>
                                    {{ $template->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border-2 border-slate-100">
                        <input type="checkbox" name="is_active" id="is_active" {{ $form->is_active ? 'checked' : '' }} class="w-5 h-5 text-primary border-slate-300 rounded focus:ring-primary">
